...
The \textit{Tools} menu provides useful functionality such as the ability to Lock a track (see \S\ref{TrackLocking}) and to copy the track's URL to the clipboard.  Track types which have the ability to filter their contents add a Filter… item to the Tools menu, which invokes a dialog to set filter parameters.
...
In addition, Interval Tracks also have a number of Appearance options.  Depending on the s

\subsubsection{Alignment Tracks}

Alignment tracks display data from a BAM file.  They present reads in a choice of seven different display modes:
...
Variant tracks display structural variation data from Tabix-formatted VCF files.  The vertical axis is used to indicate the individuals whose data makes up the file.  Data from all open variant tracks is aggregated in the Variation sheet described in \S\ref{Variants}.
...
\begin{table}[h]
\caption{Specifying Ranges in the Location Field}
\begin{center}
\begin{tabular}{|l l|}
\hline
\multicolumn{2}{|c|}{Clicking the downward-pointing triangle to reveal a menu of chromosomes.}\\
\hline
\multicolumn{2}{|c|}{Type a gene name into the text field or}\\
\multicolumn{2}{|c|}{type a partial gene name and hit {\sc tab} to pop up a menu of matching genes.}\\
\hline
\multicolumn{2}{|c|}{Type a range specification into the text field:}\\
...
\caption{Changing the visualisation mode.}
\label{changevizmode}
\end{center}
\end{figure}
