...
New in Savant 2.0 is are ``tool plugins'', which provide a way to integrate external applications into Savant.  To the end-user, these are functionally similar to panel plugins, but where panel plugins consist of Java code packaged inside a JAR file, tool plugins consist of an XML file providing instructions on how to invoke an external application.  The format of this XML file is described in the Developer Manual.
