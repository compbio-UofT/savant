...
public final class Genome implements Serializable, GenomeAdapter {
    ...
    public byte[] getSequence(String reference, RangeAdapter range) throws IOException, InterruptedException {
        return isSequenceSet() ? sequenceTrack.getSequence(reference, range) : null;
    }
    ...
    public Cytoband[] getCytobands(String ref) {
        if (cytobands == null) {
            if (cytobandURI == null && sequenceTrack != null) {
                for (Genome g: getDefaultGenomes()) {
                    if (g.auxiliaries != null && g.auxiliaries.length > 0 && g.auxiliaries[0].uri.equals(sequenceTrack.getDataSource().getURI())) {
                        // Hey!  I recognise this track, it's one of ours.
                        cytobandURI = g.cytobandURI;
                        break;
                    }
                }
            }
