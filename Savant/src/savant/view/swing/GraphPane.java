...
public class GraphPane extends JPanel implements MouseWheelListener, MouseListener, MouseMotionListener, GraphPaneChangeListener {
    ...
    private static Image lock = SavantIconFactory.getInstance().getIcon(SavantIconFactory.StandardIcon.LOCK).getImage();
    ...
    // TODO: draw in a separate layer
    private void drawTrackLabel(Graphics2D g2, String trackLabel) {
        //g2.translate(0, 0);
        FontMetrics fm = g2.getFontMetrics();
        Rectangle2D r = fm.getStringBounds(trackLabel, g2);
        int b = 1;
        g2.setColor(new Color(255,255,255,180));
        g2.fillRoundRect(b, b, (int) r.getWidth()+2*b, (int) fm.getHeight()+2*b, 5,5);
        g2.setColor(Color.darkGray);
        g2.drawString(trackLabel, b+b, b+(int)fm.getHeight()-fm.getMaxDescent());
    }
    ...
    public Dimension render(Graphics g, Range xRange, Range yRange) {
        ...
        if(sameRange && sameMode && sameSize && sameRef && !renderRequired && withinScrollBounds){
            ...
            if (nothingRendered && message != null) {
                ...
                //g3.setFont(BrowserSettings.getTrackFont());
                //trackLabel = trackLabel.trim();
                //drawTrackLabel(g3,trackLabel);
            }

            //System.out.println("Graphpane " + this.toString() + " repainted");
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (isLocked()) {
            int w = 30, h = w;
            g.drawImage(lock, 2, this.getHeight()-2-h, w, h, null);
    ...
    private void renderSides(Graphics g) {
        ...
        Color c = Color.lightGray;//ColourSettings.getFrameBackground();

        Graphics2D g2 = (Graphics2D)g;

        GradientPaint gpleft = new GradientPaint(buffer,0,t,0,0,c);
        g2.setPaint(gpleft);
        g2.fillRect(-this.getWidth(), 0, buffer+this.getWidth(), this.getHeight());

        GradientPaint gpright = new GradientPaint(this.getWidth()-buffer,0,t,this.getWidth(),0,c);
        g2.setPaint(gpright);
        g2.fillRect(this.getWidth()-buffer, 0, buffer+this.getWidth(), this.getHeight());
         * 
         */
    ...
    public void mouseMoved( final MouseEvent event ) {
        ...
        int pixel = (int)(Math.floor(event.getX()/this.getUnitWidth())*this.getUnitWidth()); // round down instead of to the nearest position
        int position = MiscUtils.transformPixelToPosition(pixel, this.getWidth(), this.getHorizontalPositionalRange());
        GraphPaneController.getInstance().setMouseXPosition(position);
        if (isOrdinal()) {
            GraphPaneController.getInstance().setMouseYPosition(-1);
        } else {
