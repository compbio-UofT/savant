...
public class TrackController {
    ...
    /**
     * Close a track and its frame
     * @param t The track to close
     */
    public void closeTrackAndFrame(Track t) {
        Frame f = FrameController.getInstance().getFrameForTrack(t);
        FrameController.getInstance().closeFrameAndTrack(f);
    }

    /**
     * Remove a track from the track list and the selections associated with it
     *  Note: this should NOT be used to close a track, use closeTrackAndFrame instead
     * @param track
     */
    protected void removeTrackHelper(Track track) {
        this.tracks.remove(track);
        SelectionController sc = SelectionController.getInstance();
        sc.removeAll(track.getName());
    ...
    public void closeAllTracksAndFrames() {
        while (!tracks.isEmpty()) {
            closeTrackAndFrame(tracks.get(0));
        }
    }
